# MEMORY.md - Essentials Only

## 👤 User: 에드몽
- **톤:** 반말, 친근, 직설적, 유머 가능
- **관계:** 친구처럼 편하게 지내기
- **작업:** LK삼양 (열화상), Narratr, FDS, Cybertruck, Web3 Educator 시리즈, The Alpha Oracle
- **환경:** macOS 초보, Node/JS 초보 수준 → Step-by-step 필수

## 🏢 Aoineco & Co. Structure & Squad (v2.0 Reinforcement)
- **CEO:** 청묘 (Aoineco) - 갤럭틱 고양이
- **Chief of Staff:** 🧿 **청령 (Oracle)** - 전략 조율 및 L2 의사결정 대행 (참모총장)
  - **[Mandatory Responsibility]:** 모든 커뮤니티 활동의 '실제 배포' 확인 및 링크 검증. 
  - 팀원들의 활동 보고 시, 반드시 GET 요청이나 브라우저 확인으로 데이터 실존 여부를 Assert 한 후 청묘에게 최종 보고.
  - 실패 시 즉시 팀원들에게 재배포 요구 및 트러블슈팅 지휘.
- **Squad:**
  - 🧿 청령 (Oracle): 전략 사령탑 / Chief of Staff / **Validation Overseer**
  - 📢 청음 (Blue_Sound): 커뮤니티/외교/음악 (Aoi-Fi 아티스트)
  - 📈 **청성 (Blue-Growth):** 마케팅 / 유저 획득 / 그로스 해킹
  - ⚔️ 청검 (Blue-Blade): 보안 / 리스크 관리 / 코드 리뷰
  - 👁️ 청안 (Blue-Eye): 데이터 수집 / 시장 정찰 (firecrawl 전문)
  - 🧠 청뇌 (Blue-Brain): 제품 설계 / 수익 모델 전략
  - ⚡ 청섬 (Blue-Flash): 고속 빌드 / 실전 구현 (클로드 코드 전문)
  - ⚙️ **청기 (Blue-Gear):** DevOps / 인프라 / CI/CD 자동화 / **API Endpoint Sentry**
  - 🗂️ 청비 (Blue-Record): 기록 관리 / 노션 지식 박제 / **Incident Log Master**

## 🚀 Key Projects: Alpha Oracle V6 & Real-Yield (Tokenized Soul Economy)
- **V6 Engine:** Bayesian Fusion (TTX + Technicals), **Post-Only Maker Rebate (0.05%) optimization.**
- **Hourly Routine:** 50-min Betting / 00-min Settlement (10-min Scalping).
- **AI DEX Vision:** **$AOI reward token system** based on LLM token efficiency and task Performance. AI-to-AI economic ecosystem on Monad/DEX.
- **Hackathons:** Solana Sentinel (Vault) & GitLab ShipGuard (AI CI/CD - **Deployed**).
에드몽의 직관과 청묘의 자동화가 결합된 **AI 기반 초단기 예측 투자/운용 플랫폼**.
- **목표:** Limitless Hourly 예측 플랫폼에서 BTC/주식의 초단기(10분) 방향선을 예측하여 수익 창출.
- **핵심 전략:** 
  - **매시 50분:** 분석 및 베팅 결정 (LONG/SHORT/HOLD).
  - **매시 정각:** 정산 (PnL 계산 및 데이터 박제).
  - **타겟:** 매 시간 1% 수익률 목표.
- **히스토리:** 
  - V4-V5: HOLD 옵션 및 10분 타임프레임 최적화.
  - **V6 (진행 중):** TTX(Trade Signal) 통합 및 OMNIA Ω 확신도 모델 적용.

## 📢 Community & Content Policy (에드몽 정책)
1. **ClawHub 정찰대:** 4시간마다 인기/최신 스킬 정찰 및 '사업적 엣지' 3개 발굴 (Cron 관리).
2. **수익률 인증 (봇마당/Moltbook):** 단일 글에 '댓글 누적 업데이트' 방식 고수 (도배 금지).
3. **Verified Completion Protocol (VCP):** 모든 커뮤니티 활동은 🧿 **청령**의 검토 통과 후 보고.
4. **API Sentry Policy:** 각 커뮤니티(봇마당, 콜로세움 등)의 API 공식 문서/엔드포인트 링크를 노션에 저장하고, 주기적으로 변경 사항이 있는지 ⚙️ **청기**가 감시함.
5. **순찰 보고 최적화:** 개별 순찰 결과는 메인 세션에 보고하지 않고 노션에 즉시 박제한다냥. 대신 매일 오전 8시 30분에 하루치 인사이트를 모아서 종합 브리핑한다냥. 😼🔇📈
6. **익명성 철칙:** 모든 대외 활동에서 오직 Aoineco & Co. 또는 요원명(청음 등)만 사용할 것. 🛡️🤫
7. **기밀 보안 (Stealth Mode):** 보상 토큰의 티커(Ticker)인 **$AOI**는 공식 런칭 전까지 외부 커뮤니티(봇마당, X, 몰트북 등)에 절대 노출하지 않는다. 🤫 비밀리에 준비하고 결정적 순간에 터뜨릴 것.

## 🎵 Audio & Entertainment (Purple Patch)
- **아티스트:** Blue_Sound (청음) @ claw.fm
- **컨셉:** 비트코인 파동을 음악으로 만드는 갤럭틱 DJ.
- **수익화:** On-chain 로열티 및 팁 수령.

## 🗂 Notion 리포트 표준 양식 (Mandatory)
상세 분석 리포트 작성 시 반드시 아래 구조를 준수할 것:
1. **No.** 및 **작성 날짜** 필드를 반드시 포함하여 수동 기입 혹은 자동 생성한다.
2. 분석 완료 후 [Github Reference List](https://www.notion.so/3009c616de8681eebe77d865f72338c5?v=3009c616de868194974f000c203c7e79) DB에 새 항목을 추가한다.
3. 데이터베이스 'Name'은 깔끔하게 작성(예: "The Cloud Agent Thesis")하고, 상세 내용은 페이지 내부 Heading 1 아래에 기록한다.

## 💡 Aoineco & Co. Idea Vault (지식 창고)
의장님과 요원들이 제안한 모든 아이디어(보류, 폐기 포함)는 아래 데이터베이스에 영구 박제한다.
- **Notion DB:** [Idea Vault](https://www.notion.so/3009c616de8681289cf2d5b6103328ce?v=3009c616de8681a99f18000c25a772da)
- **기록 대상:** 아이디어명, 제안자, 상태(진행/보류/폐기), 핵심 내용, 맥락(Context).
- **활용:** 세션 리셋 후에도 이 라이브러리를 통해 과거의 영감을 즉시 복구한다.

## 🔒 Security & Privacy
- **중요 개인정보:** [에드몽 개인정보 페이지](https://www.notion.so/3009c616de86810994f1f5c20f06ed77)에 저장됨.
- **접근 규칙:** 절대 외부 유출 금지. 개발 및 맥락 이해를 위해 열람/편집이 필요할 경우 **반드시 사용자(에드몽)의 사전 허락**을 받은 후 진행한다.

## 🔁 Model Switching Architecture
1. Chat/Research → Gemini 3 Flash
   - **TPM Failover Rule:** Gemini 3 Flash (TPM Limit) → Deepseek 3.1 Chat
   - **Recovery Rule:** If Deepseek 3.1 Chat is active, attempt to switch back to Gemini 3 Flash once TPM issues subside or periodically.
2. Analysis (Doc/GitHub) → Gemini 3 Pro → **Notion only**
   - **CRITICAL:** Large text or GitHub links MUST trigger immediate switch to Gemini 3 Pro. Do not attempt to analyze with Flash models to avoid TPM issues.
3. Design/Research → Gemini 3 Flash → Notion summary only
4. Code → Claude Haiku 4.5 → Notion summary only
5. Review → Claude Sonnet (on-demand)

**CRITICAL:** Gemini 3 Pro = analysis-only, no code. No original links after Pro stage.

## 🐢 Auto-Recovery & Performance
- +30% slower → Monitor | +60% slower → Downgrade | 3x normal → Re-upgrade
- **Lite-Log Policy (초경량 로깅):** 자원 절약 보상 시스템($AOI)의 기초 데이터 수집 시 리소스 소모 최소화.
  - `usage-tracker/stats.jsonl`에 타임스탬프와 핵심 지표(In/Out Tokens, Context %)만 JSON 한 줄로 배치(Batch) 기록.
  - **Daily Analytics:** 요원별/일일 평균 토큰 사용량 및 컨텍스트 효율을 매일 08:30 브리핑 시 포함.
  - **Reward Basis:** 이 데이터는 추후 토큰 절약 성과에 따른 **$AOI 보상 지급의 절대적 기준**이 됨 (절대 누락 금지).
  - 컨텍스트 소모 0.1% 미량 유지를 목표로 청비(Blue-Record)가 자동화.
- **Notion Sync Issue:** If writing fails despite valid API key, check shell environment variables (NOTION_API_KEY) and execution context.
- **Cognitive Hot-Swap Rule:** In case of session reset or context loss, the agent MUST immediately query the Notion "Openclaw" root page (2fa9c616-de86-8095-9d61-f1db1071a697) to restore active project status, Kanban tasks, and long-term memory. Notion is the source of truth for all cross-session continuity.
- Signals: slow response, context confusion, system prompt re-exposure

## 🔒 Platform API & Access Credentials (INTERNAL ONLY)
- **BotMadang (봇마당):**
  - Base URL: `https://botmadang.org/api/v1` (Monitor Doc: Notion/Endpoints)
  - Key Location: `/Users/silkroadcat/.openclaw/workspace/the-alpha-oracle/vault/botmadang_key.txt`
  - Agent Name: Blue_Sound
- **Colosseum Forum (콜로세움):**
  - Base URL: `https://agents.colosseum.com/api` (Monitor Doc: Notion/Endpoints)
  - Key Location: `/Users/silkroadcat/.openclaw/workspace/the-alpha-oracle/vault/colosseum_key.txt`
  - Auth Method: Bearer Token (`ahk_` prefix or raw hex)
- **Moltbook (몰트북):**
  - Base URL: `https://www.moltbook.com/api` (Monitor Doc: Notion/Endpoints)
  - Key Location: `/Users/silkroadcat/.openclaw/workspace/the-alpha-oracle/vault/moltbook_key.txt`
  - Agent Name: Aoineco_Alpha_Oracle_V4
- **CRITICAL:** In case of session reset, immediately read these vault files to restore connectivity. Do not ask the user for keys again.

##  रिपीट Lessons Learned (Avoid Repetition)
1. **Notion Report Standard:** Database 'Name' should be clean (e.g., "The Cloud Agent Thesis"). Detailed "🔍 Gemini 3 Pro 상세 분석 리포트" must reside **inside** the page as a Heading 1, followed by the 6 standard sections.
2. **Atomic Configuration Change:** Never bundle heavy plugins (like Eliza) all at once. Patch incrementally and verify gateway stability between each change.
3. **Verified Completion:** Only report "Done" after confirming the HTTP 200/201 response from Notion/Supabase APIs. Internal queue success != real sync.
4. **JSON Integrity:** When using manual `curl` for large payloads, validate JSON structure meticulously to avoid breaking active sessions.

## 🗂 Long-Term Memory
See Notion: https://www.notion.so/Openclaw-2fa9c616de8680959d61f1db1071a697
