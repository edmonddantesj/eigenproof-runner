...
def update_notion_full_report(no, title, summary, key_points, observation, page_id):
    ...
    # Payload for properties and internal content (blocks)
    payload = {
        "properties": {
        ...
        "children": [
            {
                "object": "block",
                "type": "heading_1",
                "heading_1": { "rich_text": [{"text": {"content": f"🔍 Gemini 3 Pro 상세 분석 리포트"}}] }
            },
            {
                "object": "block",
                "type": "paragraph",
                "paragraph": { "rich_text": [{"text": {"content": f"한 줄 요약: {summary}"}, "annotations": {"bold": True}}] }
            },
            {
                "object": "block",
                "type": "heading_2",
                "heading_2": { "rich_text": [{"text": {"content": "核心 구성 요소"}}] }
            },
            {
                "object": "block",
                "type": "paragraph",
    ...
    for page in pages:
        ...
        if name_prop:
            ...
            if name in data_map:
                ...
                print(f"✅ Fully Populated '{name}' (No.{start_no + list(data_map.keys()).index(name)}): {status}")
