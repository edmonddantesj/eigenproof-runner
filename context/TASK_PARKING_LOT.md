...
- Progress:
  - 제출 완료 (영상/링크 확정 완료)
- Notes:
  - Public repo is FINAL; do not modify unless explicitly requested
- Evidence:
...
- Status: PARKED
- Progress:
  - the-alpha-oracle/submission.json 에 V3 표기 잔존(현행은 V6-only 정책)
- Needs from Edmond:
  - V6 기준 문구로 rewrite할지/아카이브로 둘지 결정
- Resume trigger:
  - "submission 문서 정리" 신호
- Resume first command:
...
- Status: ❌ CLOSED — hackathon ended, submission not made
- Deadline: Feb 20 11:59pm PT (passed)
- Requirements:
...
- Outcome:
  - Submission not made (blocked by local Docker access + time)
- Post-mortem next actions (optional):
  - If re-attempting similar hackathons: ensure Docker/Eigen toolchain ready 24h before deadline
  - Create a minimal EigenCompute proof runner template in advance
- Evidence:
...
- Status: ✅ SUBMITTED (DONE) — submission completed; backlog remains for vNext
